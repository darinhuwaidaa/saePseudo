...
avgPseudo <- function(prov, reg, sub, vill, y, x, var, N, method="REML") {
  ...
  # Calculate Weight_agr (Nij/Ni) value
  sae_vill <- sae_vill %>%
    mutate(Weight_agr = N_vill / N_sub)

  # Calculate y_agr_villsub per Subdistrict
  vill_agr_sub <- sae_vill %>%
    group_by(Province, Region, Subdistrict) %>%
    summarize(y_agr_villsub = sum(Weight_agr * Y_eblup))

  # Calculate variance per Subdistrict
  var_agr_villsub <- sae_vill %>%
    group_by(Province, Region, Subdistrict) %>%
    summarise(var_agr_villsub = sum(Weight_agr^2 * MSE_eblup))
  ...
  sub_agr_reg <- vill_agr_sub %>%
    group_by(Province, Region) %>%
    summarize(y_agr_subreg = sum(Weight_agr * y_agr_villsub))

  # Calculate var_agr_subreg
  var_agr_subreg <- vill_agr_sub %>%
    group_by(Province, Region) %>%
    summarise(var_agr_subreg = sum(Weight_agr^2 * var_agr_villsub))
  ...
  # Calculate Weight_agr (Nij/Ni) value
  sub_agr_reg <- sub_agr_reg%>%
    mutate(Weight_agr = N_reg / N_prov)
  ...
  reg_agr_prov <- sub_agr_reg %>%
    group_by(Province) %>%
    summarize(y_agr_regprov = sum(Weight_agr * y_agr_subreg))

  # Calculate var_agr_regprov
  var_agr_regprov <- sub_agr_reg %>%
    group_by(Province) %>%
    summarise(var_agr_regprov = sum(Weight_agr^2 * var_agr_subreg))
